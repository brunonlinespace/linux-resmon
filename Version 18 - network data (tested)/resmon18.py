...
class ResMonLinux(QMainWindow):
    def __init__(self):
        ...
        self.prev_io = {}
        self.prev_net_io = {} # Tracker for network I/O rates
        self.proc_cache = {} 
        ...
        # Restore user settings (includes theme selection) before starting timers
    ...
    def create_net_widget(self, splitter=None):
        box = QGroupBox()
        layout = QVBoxLayout(box)
        layout.addLayout(self.create_panel_header("Network Traffic", box, splitter))
        flt = QLineEdit()
        flt.setPlaceholderText("Filter Network by Interface...")
        tbl = QTableWidget(0, 5)
    ...
    def setup_overview_tab(self):
        ...
        # Set Object Names for Settings Retrieval
    ...
    def restore_settings(self):
        ...
        # 1. Restore Window Size & Position
    ...
    # --- THE DELTA UPDATE RECYCLER ---
    def update_cell(self, tbl, row, col, text, raw_val=None):
        """Updates an existing cell to save memory, or creates one if it doesn't exist."""
        item = tbl.item(row, col)
        
        # If we need this cell to sort by a hidden raw number (like bytes or CPU %)
