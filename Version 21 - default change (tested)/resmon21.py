...
class ResMonLinux(QMainWindow):
    def __init__(self):
        ...
        # Set default to processes instead of interfaces
        self.net_view_mode = "processes" 
        self.net_toggle_buttons = []
        ...
        # Ensure toggle button text matches the active view mode after restoring settings
        self.update_network_toggle_buttons_text()
    ...
    def update_network_toggle_buttons_text(self):
        btn_text = "View: Interfaces" if self.net_view_mode == "processes" else "View: Processes"
    ...
    def create_net_widget(self, splitter=None):
        ...
        btn_toggle = QPushButton("View: Interfaces") # Initial text; updated right after
        ...
        tbl = QTableWidget(0, 6) # Default to processes columns
        tbl.setHorizontalHeaderLabels(["Process Name", "PID", "Local Address", "Remote Address", "Status", "Protocol"])
        tbl.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        tbl.setSortingEnabled(True)
    ...
    def restore_settings(self):
        ...
        if settings.contains("netViewMode"):
            self.net_view_mode = settings.value("netViewMode", "processes", type=str)
            # Re-apply column setup if settings restored "interfaces"
            if self.net_view_mode == "interfaces":
                for tbl in [self.ov_net_table, self.net_table]:
                    tbl.clear()
                    tbl.setColumnCount(5)
    ...
    def closeEvent(self, event):
        ...
        settings.setValue("netViewMode", self.net_view_mode)
    ...
    def update_net_data(self, flt, tbl):
        ...
        if self.net_view_mode == "interfaces":
            ...
            try:
                ...
                for nic, io in net_io.items():
                    ...
                    if nic in self.prev_net_io:
                        old_sent, old_recv, old_time = self.prev_net_io[nic]
                        dt = now - old_time
                        up_rate = max(0, (bytes_sent - old_sent) / dt) if dt > 0 else 0
                        down_rate = max(0, (bytes_recv - old_recv) / dt) if dt > 0 else 0
                    else:
                        up_rate, down_rate = 0, 0
                    self.prev_net_io[nic] = (bytes_sent, bytes_recv, now)
                    interfaces.append((nic, up_rate, down_rate, bytes_sent, bytes_recv))
            ...
            for row, (nic, up_rate, down_rate, total_up, total_down) in enumerate(interfaces):
                self.update_cell(tbl, row, 0, nic)
                self.update_cell(tbl, row, 1, self.format_rate(up_rate), up_rate)
                self.update_cell(tbl, row, 2, self.format_rate(down_rate), down_rate)
                self.update_cell(tbl, row, 3, self.format_bytes(total_up), total_up)
                self.update_cell(tbl, row, 4, self.format_bytes(total_down), total_down)
                
        else:
            connections = []
            def parse_conn(conn, pid_hint=None):
                if not getattr(conn, 'raddr', None): return
                pid = getattr(conn, 'pid', pid_hint)
                name = "System/Kernel"
                if pid and pid in self.proc_cache:
                    try: name = self.proc_cache[pid].name()
            ...
            for row, (name, pid, laddr, raddr, status, proto) in enumerate(connections):
                self.update_cell(tbl, row, 0, name)
                self.update_cell(tbl, row, 1, str(pid), pid)
                self.update_cell(tbl, row, 2, laddr)
                self.update_cell(tbl, row, 3, raddr)
                self.update_cell(tbl, row, 4, status)
                self.update_cell(tbl, row, 5, proto)
