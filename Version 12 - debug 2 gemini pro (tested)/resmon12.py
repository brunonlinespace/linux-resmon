...
class ResMonLinux(QMainWindow):
    def __init__(self):
        ...
        # Trigger immediate refresh when a user switches tabs
    ...
    def update_net_data(self, flt, tbl):
        ...
        try:
            # Try global connection fetch (usually requires sudo)
            for conn in psutil.net_connections(kind='inet'):
                parse_conn(conn)
        except psutil.AccessDenied:
            # Non-root fallback: iterate over processes the user has permission to view
            for pid, proc in self.get_active_processes():
                try:
